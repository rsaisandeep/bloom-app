...
// The "ⓘ" button — explains what Bloom gives you for what you log,
// and points to the article library.
export default function InfoModal() {
  const router = useRouter();
  const [open, setOpen] = useState(false);

  const youLog = ['Period start & flow', 'Mood & energy', 'Cramps & bloating', 'Sleep & cravings'];
  const youGet = [
    { icon: '🌙', t: 'Your phase & cycle day', s: 'Adapts to your real period & cycle length' },
    { icon: '🗓', t: 'Period & fertile predictions', s: 'A safe date range in PCOS mode' },
    { icon: '✅', t: 'Daily focus tasks', s: 'Tailored to your phase & today’s symptoms' },
    { icon: '📊', t: 'Science-backed reports', s: 'Food, movement & self-care that fits the phase' },
  ];
      ...
      {open && typeof document !== 'undefined' && createPortal(
        <div onClick={() => setOpen(false)} style={{
          ...
          <div onClick={(e) => e.stopPropagation()} style={{
            width: '100%', maxWidth: 448, maxHeight: '88vh', overflowY: 'auto',
            background: 'rgba(255,255,255,0.94)',
            backdropFilter: 'blur(40px) saturate(180%)', WebkitBackdropFilter: 'blur(40px) saturate(180%)',
            borderRadius: '28px 28px 0 0', padding: '22px 20px calc(24px + env(safe-area-inset-bottom))',
            boxShadow: '0 -10px 48px rgba(110,52,130,0.22)', animation: 'floatIn .3s cubic-bezier(.22,.8,.3,1) both',
          }}>
            <div style={{ width: 40, height: 4, borderRadius: 2, background: 'rgba(165,106,189,0.3)', margin: '0 auto 16px' }} />

            <div style={{ textAlign: 'center', marginBottom: 18 }}>
              <div style={{ fontSize: 36 }}>🌸</div>
              <p style={{ margin: '4px 0 2px', fontSize: 20, fontWeight: 800, color: '#1C0B2E' }}>How Bloom works</p>
              <p style={{ margin: 0, fontSize: 13, color: '#8A6A9A' }}>The more you log, the smarter it gets.</p>
            </div>
